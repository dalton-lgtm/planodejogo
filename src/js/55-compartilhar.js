...
PJ.copiar = function (txt, campo) {
  var deuCerto = function () { PJ.aviso('Lista copiada. Agora é só colar no WhatsApp.'); };
  try {
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(txt).then(deuCerto, function () { selecionar(campo); });
      return;
    }
...
function selecionar(campo) {
  ...
  try {
    ...
    PJ.aviso('Lista copiada. Agora é só colar no WhatsApp.');
  } catch (err) {
